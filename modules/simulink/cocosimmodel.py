class CoCoSimModel:
    ...
    def getBlockById(self, blockid):
        # block id can be either the Handle or the Origin_path
        result = {}
        allBlocks = self.getAllBlocks()
        for block in allBlocks:
            if block.get("Handle") == blockid or block.get("Origin_path") == blockid:
                result = block
                break
        return result
    ...
    def getModelName(self):
        # implemented
        metaData = self.getModelMetaData()
        modelName = metaData.get("file_path")
        return modelName if modelName is not None else ""
    ...
    def packBlockForTransformation(self, blockid):
        ...
        blockForTransformation["internalstatevariable"] = self.simulinkModelJson["internalstatevariables"].get(
            blockid, "")
        if len(outConns) > 0:
